...
MX (Mail eXchange)
NS (Name Server) QNAME AND RDATA
PTR (Pointer)
...
finalqname = "" # The string to be returned for use in the next method question section.
...
print(f'QDCOUNT: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2

print(f'ANCOUNT: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2

print(f'NSCOUNT: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2

print(f'ARCOUNT: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2

name, index = parse_name(index, response)
print(f'QNAME: {name}')

print(f'QTYPE: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2

print(f'QCLASS: {int.from_bytes(response[index : index + 2], byteorder="big", signed=False)}')
index += 2
